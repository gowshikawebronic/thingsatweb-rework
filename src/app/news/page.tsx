...
function NewsContent() {
    ...
    // Scroll to section when category changes via URL
    useEffect(() => {
        const categoryParam = searchParams.get("category");
        if (categoryParam) {
            setSelectedCategory(categoryParam);
            if (resultsRef.current) {
                const offset = resultsRef.current.getBoundingClientRect().top + window.scrollY - 120;
    ...
    // --- PAGINATION: Scroll exactly to the top of the results section ---
    const goToPage = (page: number) => {
        setCurrentPage(page);
        if (resultsRef.current) {
            // Calculates exact position minus 120px padding for your fixed navbar
            const offset = resultsRef.current.getBoundingClientRect().top + window.scrollY - 120;
            window.scrollTo({ top: offset, behavior: "smooth" });
        }
    };
...
export default function NewsPage() {
    return (
        <main className="min-h-screen bg-transparent font-sora selection:bg-brand-blue selection:text-white overflow-clip">
