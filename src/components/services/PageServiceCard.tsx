...
import { Check, ArrowRight, Layers, X, ArrowUpRight } from "lucide-react";
...
export const PageServiceCard = ({ service, index, onCtaClick, reversed = false }: PageServiceCardProps) => {
  ...
  // The dynamic link to explore this specific service
  ...
  const getTechPosition = (idx: number, isExpanded: boolean) => {
    ...
    if (!isExpanded) {
      if (idx === 0) return { x: "80%", y: "-80%", opacity: 1, scale: 0.9, rotate: 6, zIndex: 1 };
      if (idx === 1) return { x: "-80%", y: "80%", opacity: 1, scale: 0.9, rotate: -6, zIndex: 2 };
      return { x: "0%", y: "0%", opacity: 0, scale: 0.5, rotate: 0, zIndex: 0 };
    }

    // 2. EXPANDED STATE: The Perfect Non-Overlapping Orbit
    const expandedPositions = [
      { x: "135%", y: "-135%", rotate: 6 },
      { x: "-135%", y: "135%", rotate: -6 },
      { x: "-135%", y: "-135%", rotate: -8 },
      { x: "135%", y: "135%", rotate: 8 },
      { x: "0%", y: "-180%", rotate: 0 },
      { x: "0%", y: "180%", rotate: 0 },
      { x: "-190%", y: "0%", rotate: -12 },
  ...
  return (
    <div className="group relative w-full py-16 lg:py-20">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-12 items-center">
        ...
        <div className={`lg:col-span-5 flex flex-col gap-8 order-2 z-20 ${reversed ? "lg:order-2 lg:pl-10" : "lg:order-1 lg:pr-10"}`}>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-6">
            ...
            <Link
              href={serviceUrl}
              className="h-12 px-6 flex items-center justify-center gap-2 border border-foreground/10 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm rounded-full text-xs font-bold tracking-widest uppercase transition-all duration-300 text-foreground hover:bg-foreground/5 hover:-translate-y-1"
            >
