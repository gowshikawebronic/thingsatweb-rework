...
export default function Hero() {
    ...
    return (
        <section className="relative bg-transparent w-full min-h-screen flex items-center overflow-hidden py-32 lg:pt-20 lg:pb-10">
            ...
            <div className="absolute inset-0 z-[0] pointer-events-none flex flex-col items-center justify-center overflow-hidden mix-blend-overlay dark:mix-blend-lighten">
                <h2 className="text-foreground/[0.04] dark:text-foreground/[0.02] font-display font-black text-[100px] sm:text-[180px] lg:text-[250px] xl:text-[320px] uppercase tracking-tighter leading-[0.8] text-center select-none w-full whitespace-nowrap">
                    THINGS <br /> AT WEB
                </h2>
            </div>

            {/* --- DESKTOP SOCIAL ICONS (Left Fixed) --- */}
            <motion.div
            ...
            <div className="container-custom relative z-[4] w-full h-full flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-8 lg:pl-8">

                {/* ==========================================
                    LEFT/TOP SIDE: MINIMAL TEXT 
                ========================================== */}
                <motion.div
                    initial={{ x: -50, opacity: 0 }}
                    animate={{ x: 0, opacity: 1 }}
                    transition={{ duration: 0.8, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
                    className="w-full lg:w-1/2 max-w-2xl flex flex-col items-start z-10"
                >

                    {/* Animated Heading */}
                    <h1 className="font-display text-foreground font-black text-4xl sm:text-6xl lg:text-7xl xl:text-8xl leading-[1.1] tracking-tight mb-6">
                        Are You Looking <br />
                        For <RotatingText
                    ...
                    {/* Minimal Paragraph */}
                    <p className="text-foreground/70 text-base sm:text-lg md:text-xl leading-relaxed mb-8 max-w-xl font-medium">
                        ...
                        <span className="block mt-2 sm:mt-4 text-foreground font-bold">
                    ...
                    <div className="hidden lg:flex flex-row items-center gap-4 w-full">
                        <button
                            ...
                            className="btn-ripple h-14 px-8 cursor-pointer text-white font-bold transition-all flex items-center justify-center gap-3 group shadow-[0_10px_30px_rgba(14,173,43,0.2)] hover:shadow-[0_15px_40px_rgba(14,173,43,0.3)] duration-300 ease-out bg-gradient-green rounded-xl hover:-translate-y-1"
                        >
                            Start Your Project
                        ...
                        <button
                            onClick={() => setIsModalOpen(true)}
                            className="h-14 px-8 cursor-pointer font-bold transition-all duration-300 ease-out border-2 border-brand-blue/30 text-brand-blue bg-white/50 dark:bg-gray-900/50 backdrop-blur-md rounded-xl hover:bg-brand-blue/10 hover:border-brand-blue hover:-translate-y-1 flex items-center justify-center gap-2"
                        >
                ...
                {/* ==========================================
                    RIGHT/MIDDLE SIDE: FLOATING CARDS & SOCIALS (MOBILE)
                ========================================== */}
                <div className="w-full lg:w-1/2 relative h-[380px] sm:h-[500px] lg:h-[600px] flex items-center justify-end pr-4 sm:pr-10 lg:pr-0 mt-4 lg:mt-0">

                    {/* --- MOBILE SOCIAL ICONS (Absolute Left inside the middle container) --- */}
                    <motion.div
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ duration: 0.6, delay: 0.8 }}
                        className="absolute left-0 top-1/2 -translate-y-1/2 z-[20] flex lg:hidden flex-col gap-3"
                    >
                        {[
                            { Icon: Linkedin, href: "https://linkedin.com" },
                            { Icon: Twitter, href: "https://x.com" },
                ...
                {/* ==========================================
                    BOTTOM SIDE: MOBILE BUTTONS
                ========================================== */}
                <motion.div
                    initial={{ y: 30, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    transition={{ duration: 0.8, delay: 0.8 }}
                    className="flex lg:hidden flex-col w-full gap-4 mt-6 z-10"
                >
                    <button
                        onClick={() => setIsModalOpen(true)}
                        className="w-full h-14 cursor-pointer text-white text-xs tracking-widest uppercase font-bold transition-all shadow-xl bg-foreground rounded-full active:scale-95"
                    >
                        Start Project
                    </button>
                    <button
                        onClick={() => setIsModalOpen(true)}
                        className="w-full h-14 cursor-pointer text-xs tracking-widest uppercase font-bold transition-all border-2 border-foreground/10 text-foreground bg-transparent rounded-full active:bg-foreground/5"
                    >
                        Free Audit
                    </button>
                </motion.div>
